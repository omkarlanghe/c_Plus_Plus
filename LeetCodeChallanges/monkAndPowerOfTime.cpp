...
void rotateArrByOneEle(int*,int);
int totalTime = 0;
...
int main() {
    ...
    shiftArray(callingOrder, idealOrder, size);
    
    for(int k = 0 ; k < size ; k++) {
        if(callingOrder[k] == idealOrder[k]) {
            callingOrder[k] = 0;
            totalTime++;
        }
    }
...
void shiftArray(int *cOrder, int *iOrder, int size) {
    while(cOrder[0] != iOrder[0]) {
        rotateArrByOneEle(cOrder,size);   
    }
}

void rotateArrByOneEle(int *arr, int size) {
    int temp = arr[0], j;
    for(j = 0 ; j < size - 1 ; j++) {
        arr[j] = arr[j + 1];
    }
